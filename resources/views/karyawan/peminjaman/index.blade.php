            ...
            <table class="min-w-full bg-white border">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="py-2 px-4 border-b text-left w-48">Catatan IT</th>
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="py-3 px-4 border-b align-top text-center">{{ $loan->user->name }}</td>
                        <td class="py-3 px-4 border-b align-top text-center">{{ $loan->jenis_barang }}</td>
                        <td class="py-3 px-4 border-b align-top text-center">{{ $loan->jumlah_barang }}</td>
                        <td class="py-3 px-4 border-b align-top text-center">{{ \Carbon\Carbon::parse($loan->tanggal_peminjaman)->format('d M Y') }}</td>
                        
                        <td class="py-3 px-4 border-b align-top text-left text-gray-700 text-sm">
                        ...
                        <td class="py-3 px-4 border-b align-top text-center">
                            <span class="px-3 py-1 rounded-full text-xs font-bold inline-block whitespace-nowrap
                                ...
                                {{ $loan->status === 'Dikembalikan' ? 'bg-blue-100 text-blue-700' : '' }}
                                {{ in_array($loan->status, ['Menunggu Verifikasi', 'Menunggu Pengembalian']) ? 'bg-yellow-100 text-yellow-700' : '' }}
                            ">
                                {{ $loan->status }}
                            </span>
                        </td>
                    ...
                    @if($loans->isEmpty())
                    <tr>
                        <td colspan="6" class="text-center py-6 text-gray-500 italic border-b">
